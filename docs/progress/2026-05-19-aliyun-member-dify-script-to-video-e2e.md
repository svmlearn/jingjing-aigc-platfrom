...
owner 生成团队本周内容 -> Dify 写回 daily task 视频脚本 -> 成员端上传素材 -> 点击 AI 剪辑 -> 直接复用 Dify draft/variant 创建 `video_edit_job` -> worker/FireRed normal no-voiceover 成片 -> `video-results/*` preview/download 200。
