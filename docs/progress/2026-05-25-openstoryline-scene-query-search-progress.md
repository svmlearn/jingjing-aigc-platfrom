...
- Kept the FireRed worker prompt minimal:
  - If uploaded media is not enough for locked script scenes, it may use the current merchant private media search capability.
  - If `sceneAssetQueries` exists, the final video should cover the locked script scene by scene.
- Removed prompt-level source/security/failure instructions from the new prompt text:
