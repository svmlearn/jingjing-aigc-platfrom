...
### 5.5 内容日历生成团队内容后的修改规则

用户在咨询页点击「生成团队本周内容」后，系统应记录：当前这份营销内容日历已经被用于生成团队内容。

这个记录不是“锁死日历”，也不是让代码直接拦截用户修改。它的作用是把事实传给 AI 咨询 Agent，让 Agent 在后续对话中自行判断是否需要先询问用户确认。

产品规则：

- `update_content_calendar` 仍应作为可用写入工具保留，不能因为本轮或历史已经调用过一次，就从 `availableTools` 中移除。
- 点击「生成团队本周内容」并成功创建批次后，要给当时的日历版本打上生成标记。
- 后续用户要求修改标题、摘要、日期、内容类型、选题角度或整份营销日历时，Agent 应能看到：
  - 当前日历版本 ID。
  - 已生成团队内容所基于的日历版本 ID。
  - 生成批次 ID。
  - 生成时间。
  - 生成任务数量。
- 如果“当前日历版本”和“已生成团队内容所基于的版本”一致，说明当前日历已经生成过团队内容。Agent 应先告诉用户：继续修改会影响已生成的团队内容，可能需要重新生成或同步修改对应内容，再询问用户是否确认。
- 如果两个版本不一致，说明日历已经在生成后被修改过，但团队内容还没有基于新版本重新生成。Agent 应提示“当前日历与已生成内容不一致”，再根据用户意图判断是继续修改、重新生成，还是仅做说明。
- 不要在代码里硬编码“禁止修改”“自动确认”“自动回滚”。代码只负责提供事实状态、更新状态和执行工具；是否追问、如何说明影响、是否继续调用 `update_content_calendar` 由 LLM 根据上下文判断。

建议状态字段：

- `contentCalendarGeneration.currentRevisionId`：当前日历版本。
- `contentCalendarGeneration.generatedFromRevisionId`：最近一次团队内容生成所使用的日历版本。
- `contentCalendarGeneration.status`：`draft`、`generated`、`modified_after_generation`。
- `contentCalendarGeneration.generatedBatchId`：最近一次团队内容生成批次。
- `contentCalendarGeneration.generatedAt`：最近一次团队内容生成时间。
- `contentCalendarGeneration.generatedByUserId`：触发生成的用户。
- `contentCalendarGeneration.generatedJobCount`：生成任务数量。

验收标准：

- 同一轮对话中，Agent 调用过 `update_content_calendar` 后，`availableTools` 里仍可再次出现 `update_content_calendar`。
- 点击「生成团队本周内容」成功后，后端持久化本次生成所基于的日历版本和批次信息。
- 下一轮对话时，Agent 的受控上下文里能看到日历生成状态。
- 用户要求修改已经生成过团队内容的日历时，Agent 不应直接声称已修改；应先说明影响并询问确认，除非用户本轮已经明确确认继续修改。
...
3. 再读 `docs/架构规范/2026-04-28-current-architecture.md`，确认当前技术边界。
