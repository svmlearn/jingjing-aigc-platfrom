...
3. 平台知识上传后如果直接进入全局检索，会影响线上咨询稳定性。
4. 管理员缺少复制线上 Agent 做测试的安全路径。
5. 测试台需要能看清本轮用了哪些 prompt、skill、knowledge、memory 和 tools。
6. 商家侧真实使用未来会受会员与积分影响，需要现在预留统一的权益 / 积分 gate。

## 3. 产品目标

V2.2 的目标是让平台管理员能安全地配置、测试、发布咨询 Agent 能力，同时不破坏当前线上商家端咨询稳定性。

核心目标：

1. 以 `Agent 容器` 为主体组织 System Prompt、Skill、Knowledge Set。
2. 管理员可以复制线上 Agent，形成测试 Agent，再通过测试台验证。
3. 线上商家端始终只读取明确绑定的 `当前线上咨询 Agent`。
4. Skill 是可控 prompt 片段，不开放底层 tool 权限。
5. Knowledge 先组织为知识集，再挂载到 Agent。
6. 真实咨询记录轻量配置快照，便于后续排查。
7. 会员与积分作为横切底座预留，不写死到 Agent 配置中。
...
- 平台管理员：配置 Agent、Skill、Knowledge、测试台、线上绑定。
...
Route Binding    线上入口到 Agent 的绑定，例如 consultation_default -> agent_id。
...
系统预置初始咨询 Agent、基础平台知识集，并建立 `consultation_default -> agent_id` 的线上绑定。
...
管理员复制线上 Agent 为测试 Agent，在测试台验证 prompt、skill、knowledge 效果，再决定发布配置或切换线上绑定。

### 阶段五：商家端真实运行

商家端调用当前线上咨询 Agent。如果 Agent 禁用，展示「服务维护中」。真实调用经过工具调用层、配置快照、会员 / 积分 gate。
...
flowchart TD
  ...
  E --> F{线上绑定是否有效}
  F -->|无绑定或 Agent 不存在| F1[后台提示需要设置线上咨询 Agent]
  F -->|Agent disabled| F2[商家端显示服务维护中]
  F -->|有效| G[管理员配置 Agent]
  ...
  K -->|效果不满意| G
  K -->|满意| L[发布配置或切换线上 Agent]

  L --> M[商家端发起咨询]
  M --> N{会员权益 / 积分是否允许}
  N -->|不允许| N1[提示升级会员或积分不足]
  N -->|允许| O{线上 Agent 是否可用}
  O -->|不可用| O1[服务维护中]
  O -->|可用| P[执行 Tool Call: Skill / Knowledge / Memory]
  P --> Q{工具调用是否成功}
...
- `enabled`：Agent 可用，但不等于正在服务商家端。
- `disabled`：Agent 禁用；如果仍被线上绑定引用，商家端显示「服务维护中」。
- 是否线上由 `consultation_default -> agent_id` 决定，不由 Agent 状态单独决定。
...
- 作为平台管理员
- 我希望系统有一个初始咨询 Agent、一个基础平台知识集和一条线上咨询绑定
...
2. Happy Path：
   ...
   - 系统建立 `consultation_default -> 初始咨询 Agent` 的线上绑定。
3. 分支 / 异常：
   - 如果已有有效 Agent 和线上绑定，不重复创建。
   - 如果基础平台知识集为空，允许存在，但后台显示「暂无知识文档」。
   - 如果线上绑定指向不存在的 Agent，后台提示「当前线上咨询 Agent 未正确配置」。
   - 如果线上绑定指向 disabled Agent，商家端显示「服务维护中」。
...
- 场景 1：全新环境初始化
  - GIVEN 系统没有任何 Agent
  - WHEN 初始化完成
  - THEN 系统存在一个初始咨询 Agent、一个基础平台知识集和一条 `consultation_default` 绑定

- 场景 2：已有配置环境初始化
  - GIVEN 系统已有有效 Agent 和线上绑定
  - WHEN 初始化逻辑执行
...
- 以便我能区分线上 Agent、测试 Agent 和禁用 Agent
...
2. Happy Path：
   - 页面展示 Agent 列表。
   - Agent 详情展示基础信息、System Prompt、挂载 Skills、挂载 Knowledge Sets、服务开关和线上绑定状态。
   - `draft / enabled / disabled` 表示 Agent 生命周期。
   - 线上服务由 `consultation_default -> agent_id` 决定。
3. 分支 / 异常：
   - Agent 为 `draft` 时，不允许设为线上 Agent。
   - Agent 为 `disabled` 时，如果仍被线上绑定引用，后台显示风险提示，商家端显示「服务维护中」。
   - Agent 没有挂载任何 Knowledge Set 时，允许保存，但提示「Knowledge 服务不会检索平台知识」。

**页面布局线框图**

```text
+--------------------------------------------------------------------------------+
| Agent 配置                                      [复制 Agent] [设为线上] [保存] |
+--------------------------+-----------------------------------------------------+
| Agent 列表               | 基础信息                                            |
|                          | Agent ID: as_xxx（不可改）                          |
| * 初始咨询 Agent   线上  | 名称: [初始咨询 Agent                              ] |
|   测试 Agent B    draft  | 角色: [本地生活商家内容咨询顾问                    ] |
|   旧 Agent        禁用   | 状态: [enabled v]                                   |
|                          |                                                     |
|                          | System Prompt                                       |
|                          | [ 草稿 / 生效版本 / 历史版本 tabs ]                  |
|                          |                                                     |
...
- 场景 2：draft Agent 设置线上
  - GIVEN Agent 状态为 draft
  - WHEN 管理员点击设为线上
  - THEN 系统阻止操作并提示「草稿 Agent 不能设为线上」

- 场景 3：disabled Agent 被线上引用
  - GIVEN 线上绑定指向 disabled Agent
  - WHEN 管理员查看 Agent 配置
  - THEN 后台展示风险提示，商家端展示「服务维护中」
...
- 场景 1：保存草稿
  - GIVEN 管理员编辑 Prompt
  - WHEN 点击保存草稿
  - THEN 系统保存 draft 版本，线上 active 版本不变
...
#### US-06：作为平台管理员，我希望复制 Agent 并切换线上绑定，以便安全测试新配置

**价值陈述**

- 作为平台管理员
- 我希望复制当前线上 Agent 到测试 Agent
...
2. Happy Path：
   ...
   - 新 Agent 不复制线上绑定、不复制服务启动状态、不复制测试记录和咨询记录。
   - 测试满意后，管理员可将 enabled Agent 设置为 `consultation_default`。
3. 分支 / 异常：
   - 新 Agent 名称为空时，不允许复制。
   - 新 Agent 名称重名时，提示重名并要求修改。
   - draft Agent 不允许设为线上。
   - 切换线上绑定前，系统提示影响商家端咨询入口。
...
#### US-08：作为商家用户，我希望商家端咨询入口始终使用当前线上咨询 Agent，以便获得稳定服务

**价值陈述**

- 作为商家用户
- 我希望打开咨询诊断时系统使用当前线上咨询 Agent
- 以便后台测试中的 Agent 不会影响我的真实咨询体验
...
1. 前置条件：
   - 商家进入 `/dashboard` 咨询诊断。
2. Happy Path：
   - 系统读取 `consultation_default -> agent_id`。
   - 系统确认 Agent 存在且 enabled。
   - 系统进行会员 / 积分 gate。
...
- 场景 1：正常咨询
  - GIVEN 当前线上 Agent 存在且 enabled，商家权益允许
  - WHEN 商家发送咨询消息
  - THEN 系统返回咨询回复，并记录配置快照和用量事件

- 场景 2：服务维护
  - GIVEN 当前线上 Agent 为 disabled
  - WHEN 商家进入咨询诊断
  - THEN 页面展示「咨询服务维护中，请稍后再试」
