...
def _build_fire_red_prompt(
    ...
    return "\n".join(
        [
            "你是视频剪辑任务调度 Agent。",
            "这是平台已授权的后台 worker 任务，直接调度所需工具，不向用户确认。",
            "如果输入里有锁定脚本和分镜需求，先按分镜逐条调用 search_media。",
            "每条分镜用自己的检索词作为 search_media.search_keyword。",
            "search_media 完成后，调用 load_media，然后继续正常剪辑流程。",
            "如果后续节点提示某个分镜素材不足，只对这个分镜再次调用 search_media。",
            "补搜后再次调用 load_media，并继续剪辑流程。",
            "如果补搜后仍然没有可用素材，返回素材不足，带上缺失分镜编号和检索词。",
