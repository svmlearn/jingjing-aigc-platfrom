...
class MCPConfig(ConfigBaseModel):
    ...
    extra_mcp_servers: List["ExtraMCPServerConfig"] = Field(default_factory=list)
...
class ExtraMCPServerConfig(ConfigBaseModel):
    server_name: str
    transport: Literal["sse", "streamable-http", "streamable_http", "http"]
    url: str
    timeout: float = Field(..., ge=0)
    sse_read_timeout: float = Field(..., ge=0)
