...
def _configured_server_names(request) -> list[str]:
    names: list[str] = []
    try:
        local_mcp_server = getattr(request.runtime.context.cfg, "local_mcp_server", None)
        primary = str(getattr(local_mcp_server, "server_name", "") or "").strip()
        if primary:
            names.append(primary)
        for extra in getattr(local_mcp_server, "extra_mcp_servers", []) or []:
            name = str(getattr(extra, "server_name", "") or "").strip()
            if name and name not in names:
                names.append(name)
    except Exception:
        pass
    if "storyline" not in names:
        names.append("storyline")
    return sorted(names, key=len, reverse=True)


def _split_prefixed_tool_name(tool_complete_name: str, server_names: list[str]) -> tuple[str, str]:
    server_name, tool_name = "", tool_complete_name
    for server in server_names:
        prefix = f"{server}_"
        if tool_complete_name.startswith(prefix):
            return server, tool_complete_name[len(prefix):]
    return server_name, tool_name


def _extract_mcp_summary(content: Any) -> tuple[bool, str]:
    if isinstance(content, dict):
        is_error = bool(content.get("isError", False))
        summary_block = content.get("summary")
        if isinstance(summary_block, dict):
            node_summary = summary_block.get("node_summary")
            if isinstance(node_summary, str) and node_summary.strip():
                return is_error, node_summary
        return is_error, str(content)
    ...
    is_error = bool(payload.get("isError", False) or payload.get("ok") is False)
    if is_error:
        return True, _mask_secrets(content)

    summary_block = payload.get("summary")
    if isinstance(summary_block, dict):
        node_summary = summary_block.get("node_summary")
        if isinstance(node_summary, str) and node_summary.strip():
            return False, node_summary
    elif isinstance(summary_block, str) and summary_block.strip():
        return False, summary_block
...
async def log_tool_request(request, handler):
    ...
    server_names = _configured_server_names(request)
    ...
    server_name, tool_name = _split_prefixed_tool_name(tool_complete_name, server_names)
        ...
        if additional_kwargs.get("isError") is True:
            ...
            if is_mcp_tool:
                if additional_kwargs.get("mcp_raw_text") is True:
                    ...
                    isError, summary = _extract_mcp_summary(getattr(out, "content", ""))
