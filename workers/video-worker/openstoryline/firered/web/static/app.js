...
class App {
  ...
  async _ensureSessionReady({ forceFresh = false } = {}) {
    if (!forceFresh && this.sessionId && this.ws) {
      return this.sessionId;
    }

    if (!forceFresh && this._bootstrapPromise) {
      try {
        await this._bootstrapPromise;
      } catch {}
    ...
    if (!forceFresh && this.sessionId) {
      try {
        const snap = await this.api.getSession(this.sessionId);
        await this.useSession(this.sessionId, snap);
        return this.sessionId;
      } catch (err) {
  ...
  async _handleFilesSelected(rawFiles) {
    ...
    try {
      ...
      this.ui.showToastI18n("toast.upload_failed", { msg: (e && (e.message || e)) || "session init failed" });
      setTimeout(() => this.ui.hideToast(), 1800);
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          let resp;
          let retryAfterSessionRefresh = true;
          while (true) {
            try {
              resp = await this.api.uploadMediaChunked(this.sessionId, f, {
              ...
              if (!retryAfterSessionRefresh || !/session not found/i.test(msg)) {
                throw err;
              }
              retryAfterSessionRefresh = false;
              await this._ensureSessionReady({ forceFresh: true });
            }
          }
